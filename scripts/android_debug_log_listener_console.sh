...
# 4. 启动 WebSocket 服务监听 App 连接
# --no-line：原样输出 WebSocket 消息，换行由 App 端 payload 决定（wscat 会强制每条消息后加 \n）。
# -n：stdin EOF 时不关闭连接；非交互环境用 tail 撑住 stdin。
listen_ws() {
    tail -f /dev/null | websocat -q --no-line -n -s "${PORT}"
}
